...
class TestsWorkflow(object):
    ...
    def generate_jackson_json_input(self):
        stage_vars_list = []
        for key, val in self.get_stage_vars().iteritems():
    ...
    def get_stage_vars(self):
        ...
        apis = filter(lambda api: api['name'] == self.api_gateway_name,
                      self.api_gateway_client.get_rest_apis(limit=500)['items'])
        if not apis:
