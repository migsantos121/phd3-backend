...
def view_env_wrapper(app_name, endpoint_name, *args, **kwargs):
    """
    ...
    view_environment = get_api_environment(app_name, endpoint_name)
    from django_swagger_utils.drf_server.utils.constants import CONSTANTS

    if view_environment == CONSTANTS["ENV_MOCK"]:
        view = getattr(importlib.import_module("%s.build.mock_views.%s.%s" % (app_name, endpoint_name, endpoint_name)),
                       endpoint_name)
        response_object = view(*args, **kwargs)
    elif view_environment == CONSTANTS["ENV_IMPL"]:
        view = getattr(importlib.import_module("%s.views.%s.%s" % (app_name, endpoint_name, endpoint_name)),
                       endpoint_name)
        response_object = view(*args, **kwargs)
    else:
