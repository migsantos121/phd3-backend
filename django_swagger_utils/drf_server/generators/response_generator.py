...
class ResponseGenerator(object):
    def __init__(self, paths, response, response_name, swagger_definitions, base_path, swagger_response,
                 write_file=True):
        self.paths = paths
    ...
    def write_response_file(self, context_properties):
        ...
        response_file_contents = self.get_response_file_content(context_properties)
    ...
    def get_response_context_properties(self):
        ...
        if response_schema:
            ...
            swagger_sample_json = SwaggerSampleSchema(self.swagger_definitions, response_schema)
            ...
            if isinstance(response_data, dict) or \
                    (isinstance(response_data, list) and not isinstance(response_data[0], list)):
                object_properties = self.get_object_properties(schema_properties=response_schema)
                context_properties["response_data"] = swagger_sample_json.to_json(indent=4)
                context_properties["response_data_no_indent"] = swagger_sample_json.to_json(indent=0)
                context_properties["response_serializer"] = object_properties["field_string"]
                context_properties["response_serializer_import_str"] = object_properties["serializer_import_str"]
                context_properties["response_serializer_array"] = object_properties["is_array_serializer"]

        return context_properties

    def get_object_properties(self, schema_properties):

        from django_swagger_utils.drf_server.generators.serializer_generator import SerializerGenerator
        serializer_generator = SerializerGenerator(schema_properties=schema_properties, base_path=self.base_path,
    ...
    def get_response_headers_context_properties(self):
        ...
        if headers:
            ...
            context_properties["response_headers_serializer_import_str"] = "from %s import %s" % (
                package_import_str, response_headers_serializer
            )
            context_properties["response_headers_serializer"] = response_headers_serializer
            context_properties["response_headers_example_params"] = response_header_example_params
        return context_properties
    ...
    def process_headers_serializer_data(self, headers):
        ...
        for header_name, each_header, in headers.iteritems():
            each_header_properties = self.header_field(each_header, header_name)
            header_name = header_name.replace("-", "_")
            response_header_serializer_fields["params"][header_name] = each_header_properties
            response_header_serializer_fields["example_params"][header_name] = each_header_properties["param_example"]
            response_header_serializer_fields["required_params"].append(header_name)
        ...
        package_import_str = convert_path_to_package_str(serializer_path, self.paths['base_dir'])

        self.write_response_headers_file(serializer_path, response_header_serializer_fields)

        return package_import_str, response_headers_serializer, response_header_serializer_fields["example_params"]

    def write_response_headers_file(self, serializer_path, response_header_serializer_fields):
        response_headers_serializer_file_contents = self.get_response_header_file_content(
            response_header_serializer_fields)

        from django_swagger_utils.core.utils.write_to_file import write_to_file
        write_to_file(response_headers_serializer_file_contents, serializer_path)

    @staticmethod
    def get_response_file_content(response_context):

        from django_swagger_utils.drf_server.templates.responses import RESPONSE
        serializer_template = Template(RESPONSE)
    ...
    def header_field(self, header, header_name):
        ...
        if not header_type:
            raise Exception("'type' not defined for header field :'%s' - %s" % (header_name, self.response_name))
        if header_type == "integer":
            field_properties["field_string"], field_properties["param_example"] = \
            ...
            field_properties["field_string"], field_properties["param_example"] = \
                self.get_array_field(header.get("items"), header_name, collection_format, )
        else:
            raise Exception("Invalid value for type of header field")
        return field_properties

    def get_array_field(self, array_header_field, array_name, collection_format):
        ...
        if array_header_field_type == "integer":
            child_str, example = integer_field(array_header_field, return_example=True)
        elif array_header_field_type == "number":
            child_str, example = number_field(array_header_field, return_example=True)
        elif array_header_field_type == "string":
            child_str, example = string_field(array_header_field, return_example=True)
        elif array_header_field_type == "boolean":
            child_str, example = boolean_field(array_header_field, return_example=True)
        elif array_header_field_type == "array":
            inner_collection_format = array_header_field.get("collectionFormat", "csv")
            inner_array_header_field = array_header_field.get("items")
            child_str, example = self.get_array_field(inner_array_header_field, array_name, inner_collection_format)
        else:
            raise Exception("Unsupported array field type: choices [integer, string, number, boolean, array]")
        header_field_serializer_field = "CollectionFormatField(separator='%s', child=%s)" % (separator, child_str)
    ...
    def setup_response_from_ref(self, response_ref):
        ...
        self.base_path = self.paths["global_response_dir"] + "/" + self.response_name
