    ...
    def decorator(function):
        ...
        def handler(*args, **kwargs):
            ...
            try:
                ...
                return HttpResponse(content=err.message, status=status.HTTP_400_BAD_REQUEST)
            except ClientKeyError as err:
                return HttpResponse(content=err.message, status=status.HTTP_400_BAD_REQUEST)
            except UserNotFoundException as err:
                return HttpResponse(content=err.message, status=status.HTTP_404_NOT_FOUND)
            except AuthenticationError as err:
                return HttpResponse(content=err.message, status=status.HTTP_401_UNAUTHORIZED)
            except ViewEnvironmentNotDefinedError as err:
                return HttpResponse(content=err.message, status=status.HTTP_501_NOT_IMPLEMENTED)
            except ResponseNotDefined as err:
                return HttpResponse(content=err.message, status=status.HTTP_501_NOT_IMPLEMENTED)
            except MethodNotAllowed as err:
                return HttpResponse(content=err.message, status=status.HTTP_405_METHOD_NOT_ALLOWED)
