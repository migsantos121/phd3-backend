...
class ApiDocGenerator(Generator):
    ...
    def generate_docs(self, version, apis_of_last_version):
        ...
        for path_name, path in self.parser.paths().iteritems():
            ...
            path_params, path_params_desc = self.get_params_from_properties(path, "path")
            query_params, query_params_desc = self.get_params_from_properties(path, "query")
            ...
            for method, method_props in path.iteritems():
                ...
                if (method == 'get') or (method == 'post') or (method == 'delete') or (method == 'put'):
                    ...
                    path_method_params_desc = self.merge_dicts(path_params_desc,
                                                               self.get_params_from_properties(method_props, "path")[1])
                    query_method_params = self.merge_dicts(query_params,
                                                           self.get_params_from_properties(method_props, "query")[0])
                    query_method_params_desc = self.merge_dicts(query_params_desc,
                                                                self.get_params_from_properties(method_props, "query")[
                                                                    1])
                    req_header_params, req_header_params_desc = self.get_request_headers(method_props, path)
                    res_header_params, res_header_params_desc = self.get_response_headers(method_props)
                    operation_id = method_props.get("operationId")
                    if operation_id in apis_of_last_version:
                        modified_path_name = self.get_final_path_name(path_name, query_method_params)
                        f.write('"""')
                        f.write("\n")

                        f.write("@api {" + method + "} " + "/api/" + self.app_name + modified_path_name + " " +
                                operation_id + "\n")

                        # gets the group or tag of api
                        f.write("@apiGroup " + self.app_name)
                        ...
                        req_method_parameters = self.merge_dicts(req_parameters,
                                                                 self.get_request_parameters(method_props))
                        res_method_parameters = self.get_response_parameters(method_props)
                        f.write("\n")
                        if self.check_if_request_is_single_model(req_method_parameters):
                            android_model = self.get_model_name_from_parameter(req_method_parameters)
                            self.print_api("Param", self.get_definition_parameters(android_model), f)
                        else:
                            self.print_api("Param", req_method_parameters, f)
                        self.print_required_params("Param", "PathParameter", path_method_params,
                                                   path_method_params_desc, f)
                        self.print_required_params("Param", "QueryParameter", query_method_params,
                        ...
                        f.write("\n")
                        self.print_required_params_example(path_method_params, f)

                        if self.check_if_request_is_single_model(res_method_parameters):
                            android_model = self.get_model_name_from_parameter(res_method_parameters)
                            self.print_api("Success", self.get_definition_parameters(android_model), f)
                        else:
                            self.print_api("Success", res_method_parameters, f)

                        self.print_examples_for_req_schema(definitions, method_props, f)
                        self.print_examples_for_res_schema(definitions, method_props, f)

                        flag = 0
                        for res_type in method_props.get("responses"):
                            if (str(res_type) != '200') and (str(res_type) != 'default'):
                                response = method_props.get("responses").get(res_type)
                                response_description = response.get("description")
                                schema, response_description = self.get_error_response_schema(response,
                                                                                              response_description)
                                # prints if error is specified                                                      response_description)
                                f.write("@apiError %s %s\n" % (str(res_type), response_description))
                                if schema:
                                    if flag == 0:
                                        f.write("@apiErrorExample {json} Error-Response : \n")
                                        flag = 1
                                    f.write("\tHTTP/1.1 " + res_type + " \n")
                                    swagger_sample = SwaggerSampleSchema(definitions, schema)
    ...
    def print_required_params_example(self, params, f):
       ...
       '''
        ...
        if len(params) == 0:
            return
        f.write("@apiParamExample {json} ExampleRequest:\n")
        ...
        if res_params is not None:
            for rp in res_params:
                if type(res_params[rp][0]) is not dict:
                    rp_str = rp
                    rp_type = res_params[rp][0][0].capitalize() + res_params[rp][0][1:]
                    if not res_params[rp][2]:
                        rp_str = "[%s]" % rp_str
                    f.write("@api" + api_type + self.remove_array_notation_from_name(
                    ...
                    if self.is_definition(res_params[rp][0]):
                        ...
                        self.print_api(api_type, self.get_definition_parameters(res_params[rp][0]), f,
                                       " (" + res_params[rp][0] + ")")
                else:
                    rp_str = self.remove_array_notation_from_name(rp)
                    if not res_params[rp][2]:
                        rp_str = "[%s]" % rp_str
                    f.write("@api" + api_type + self.remove_array_notation_from_name(parent) +
                            " {" + self.snake_to_camel_case(rp) + "} " +
                            rp_str + " " +
                            str(res_params[rp][1]))
                    f.write("\n")
                    self.print_api(api_type, res_params[rp][0], f, " (" + self.snake_to_camel_case(rp) + ")")


    def print_required_params(self, api_type, params_type, params, params_desc, f):
        '''
        prints parameters
        :param api_type: type of operation (get,post etc)
        ...
        :return:
        '''
            f.write("@api" + api_type + " (" + params_type + ") {" + params[param] + "} " + param + " " + params_desc[
                param] + "\n")
        ...
        if 'parameters' in method_props:
            ...
            for params in parameters:
                if '$ref' in params:
                    ...
                    for param_name, params_ref in self.parser.parameters().iteritems():
                        if (param_name == param) and (params_ref.get('in') == "body"):
                            swagger_sample = SwaggerSampleSchema(definitions, params_ref.get("schema"))
                            example_dict = swagger_sample.get_data_dict()
    ...
    def print_examples_for_res_schema(self, definitions, method_props, f):
        ...
        if "200" in method_props.get("responses"):
            ...
            description = method_props.get("responses").get("200").get('description')
            if response:
                swagger_sample = SwaggerSampleSchema(definitions, response)
                example_dict = swagger_sample.get_data_dict()
                f.write("@apiSuccessExample {json} ExampleResponse:\n")
                f.write(json.dumps(example_dict, indent=4) + "\n")
            if response1:
                api_name = self.get_name_from_reference(response1)
                for res_name, res_ref in self.parser.responses().iteritems():
    ...
    def get_response_headers(self, method_props):
        ...
        if '200' in method_props.get("responses"):
            if 'headers' in method_props.get("responses").get("200"):
                headers = method_props.get("responses").get("200").get("headers")
                for h in headers:
                    res_headers[h] = headers[h]['type']
                    res_headers_desc[h] = headers[h].get('description', "")
            elif '$ref' in method_props.get("responses").get("200"):
                res = self.get_name_from_reference(method_props.get("responses").get("200").get("$ref"))
                for res_name, resp in self.parser.responses().iteritems():
    ...
    def get_request_headers(self, method_props, path):
        ...
        req_headers = self.merge_dicts(req_headers, self.get_params_from_properties(path, "header")[0])
        req_headers = self.merge_dicts(req_headers, self.get_params_from_properties(method_props, "header")[0])
        req_headers_desc = self.merge_dicts(req_headers_desc, self.get_params_from_properties(path, "header")[1])
        req_headers_desc = self.merge_dicts(req_headers_desc,
                                            self.get_params_from_properties(method_props, "header")[1])
