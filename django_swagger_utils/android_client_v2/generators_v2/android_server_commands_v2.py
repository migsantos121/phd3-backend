...
class AndroidServerCommandsGeneratorV2(AndroidGeneratorV2):
    ...
    def generate_android_server_commands_v2(self):
        ...
        f.write("package " + self.get_import_statement_from_path_v2(self.base_path) + ".network;\n")
            ...
            for method, method_props in path.iteritems():
                if (method == "post") or (method == "get") or (method == "delete") or (method == "put"):
                    ...
                    if os.path.exists(self.base_path + "/models/requests/" + op + "/"):
                        f.write("import " + self.get_import_statement_from_path_v2(
                            self.base_path) + ".models.requests." + op + ".*;\n")
                    if os.path.exists(self.base_path + "/models/responses/" + op + "/"):
                        f.write("import " + self.get_import_statement_from_path_v2(
                            self.base_path) + ".models.responses." + op + ".*;\n")

        f.write("import " + self.get_import_statement_from_path_v2(self.base_path + "/models/common") + ".*;\n")
        f.write("import retrofit2.Call;\n")
        f.write("import retrofit2.http.Body;\n")
        f.write("import retrofit2.http.Path;\n")
        f.write("import retrofit2.http.Query;\n")
        f.write("import retrofit2.http.POST;\n")
        f.write("import retrofit2.http.GET;\n")
        f.write("import retrofit2.http.PUT;\n")
        f.write("import retrofit2.http.DELETE;\n\n")
        ...
        for path_name, path in self.parser.paths().iteritems():
            ...
            for method, method_props in path.iteritems():
                if (method == "post") or (method == "get") or (method == "delete") or (method == "put"):
                    ...
                    req_parameters = collections.OrderedDict(req_parameters.items())
                    res_parameters = collections.OrderedDict(res_parameters.items())

                    req_is_array = ""

                    if self.check_if_request_is_array_v2(req_parameters):
                        req_is_array = "[]"

                    res_is_array = ""
                    if self.check_if_response_is_array_v2(method_props):
                        res_is_array = ".List"
                    ...
                    f.write("\tCall <" +response_content + res_is_array + ">" + " " + op_lower + "(" + body_content)
                    ...
                    if path_method_params:
                        ...
                        for pp in path_method_params:
                            f.write('@Path("' + pp + '") ' + self.get_corresponding_java_data_type_v2(
                            ...
                            if counter != path_params_len - 1:
                                ...
                                if query_method_params:
                                    f.write(", ")
                            counter += 1
                    #executes if method has query parameters
                    if query_method_params:
                        ...
                        for qp in query_method_params:
                            f.write('@Query("' + qp + '") ' + self.get_corresponding_java_data_type_v2(
