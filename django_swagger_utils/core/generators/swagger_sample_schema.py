...
class SwaggerSampleSchema(object):
    ...
    def get_definition_json(self, definition_name):
        ...
        swagger_sample_json = SwaggerSampleSchema(self.definitions, definition, definition_name)
    ...
    def get_data_dict(self, schema=None):
        ...
        if schema_type in ["integer", "string", "boolean", "number"]:
            data = self.get_primitive_data_type(schema_type, format=format, enum=enum)
    ...
    def get_all_of_dict(self, all_of_props):
        ...
        for each_schema in all_of_props:
            swagger_sample_json = SwaggerSampleSchema(self.definitions, each_schema, self.definition_name)
    ...
    def object_data_dict(self, schema):
        ...
        data = {}
        for property_name, each_property in properties.iteritems():
            swagger_sample_json = SwaggerSampleSchema(self.definitions, each_property, self.definition_name)
    ...
    def to_json(self, indent=4):
        ...
        if isinstance(data, dict) or (isinstance(data, list) and not isinstance(data[0], list)):
            if indent:
