...
class AndroidRequestResponseGenerator(AndroidGenerator):
    ...
    def generate_android_request_response(self):
        ...
        if self.path_params:
            ...
            self.path_params = self.convert_data_types_to_java_format(self.path_params)
        # if query parameters exist
        if self.query_params:
            self.query_params = collections.OrderedDict(sorted(self.query_params.items()))
            self.query_params = self.convert_data_types_to_java_format(self.query_params)
        ...
        self.regenerate_model_if_array(self.req_parameters)
        self.regenerate_model_if_array(self.res_parameters)

        self.req_parameters = self.generate_nested_models(self.req_parameters, self.base_path + "/models/requests/" + self.req_name)
        self.res_parameters = self.generate_nested_models(self.res_parameters, self.base_path + "/models/responses/" + self.req_name)

        self.req_parameters_are_optional = False
        if self.check_if_parameters_are_optional(self.req_parameters):
            self.req_parameters_are_optional = True
        self.res_parameters_are_optional = False
        if self.check_if_parameters_are_optional(self.res_parameters):
            self.res_parameters_are_optional = True

        if (self.check_if_request_is_array(self.req_parameters)):
            self.req_is_array = "[]"
        ...
        if (self.check_if_response_is_array(self.method_props)):
            self.res_is_array = "[]"

        # Generating content and writing it into files
        self.req_parameters = self.convert_data_types_to_java_format(self.req_parameters)
        self.res_parameters = self.convert_data_types_to_java_format(self.res_parameters)
        ...
        if self.req_parameters and not req_is_single_model:
            android_request_file_content = self.android_request_file_contents()
            self.write_android_file(android_request_file_content, "request")

        if self.res_parameters and not res_is_single_model:
            android_response_file_content = self.android_response_file_contents()
            self.write_android_file(android_response_file_content, "response")

        android_spice_request_file_content = self.android_spice_request_file_contents(android_model, android_res_model, req_is_single_type_array=req_is_single_type_array, res_is_single_type_array=res_is_single_type_array)

        self.write_android_spice_request_file(android_spice_request_file_content)

    def android_request_file_contents(self):
        """
        ...
        android_request_context = {
            "android_common_models_import_statement": self.get_import_statement_from_path(
                self.base_path + "/models/common"),
            "android_import_statement": self.get_import_statement_from_path(
                self.base_path + "/models/requests/" + self.req_name),
            "req_parameters": self.req_parameters.items(),
        ...
        from django_swagger_utils.android_client.templates.android_request import ANDROID_REQUEST
    ...
    def android_response_file_contents(self):
        """
        ...
        android_response_context = {
            "android_common_models_import_statement" : self.get_import_statement_from_path(self.base_path + "/models/common"),
            "android_import_statement": self.get_import_statement_from_path(
                self.base_path + "/models/responses/" + self.req_name),
            "res_parameters": self.res_parameters.items(),
        ...
        from django_swagger_utils.android_client.templates.android_response import ANDROID_RESPONSE
        android_request_template = Template(ANDROID_RESPONSE)
        context = Context(android_response_context)
        return android_request_template.render(context)

    def android_spice_request_file_contents(self, android_model = "", android_res_model="", req_is_single_type_array="", res_is_single_type_array=""):
        """
        ...
        :return:
        """
        #r,res_is_single_type_array=self.filter_single_type_array(self.res_parameters)
        #print res_is_single_type_array
        ...
        android_spice_request_context = {
            ...
            "android_path_parameters": self.path_params.items(),
            "android_query_parameters": self.query_params.items(),
        ...
        from django_swagger_utils.android_client.templates.android_spice_request import ANDROID_SPICE_REQUEST
    ...
    def write_android_spice_request_file(self, android_file_content):
        """
