...
def get_stage_vars(api_name, stage, aws_region='eu-west-1'):
    ...
    response = lambda_client.get_function_configuration(FunctionName=api_name + "-" + stage)
...
def main():
    ...
    if isinstance(stage_vars, dict):
        exp = "export DJANGO_SETTINGS_MODULE=\"%s.settings.%s\"" % (old_project_name, branch)
        os.system(exp)
        print exp
        for key, val in stage_vars.iteritems():
